...
def get_brands_by_id(id_brand):
    ...
    try:
        ...
        )  # Devuelve la información en formato diccionario
        cursor.execute(
            "SELECT name FROM brand WHERE id_brand=%s AND is_active=1", (id_brand,)
...
def insert_brand(name):
    try:
        ...
        if nuevas:
            ...
            conn.commit()
            message += (
                f' Se agregaron {len(nuevas)} marca(s) nueva(s): {", ".join(nuevas)}.'
            )
        else:
...
# ! Este metodo no debería ir en el modelo brand
def get_products_by_brand(id_brand):
    try:
        ...
        cursor.execute(
            "SELECT p.*, b.name AS brand_name FROM product p JOIN brand b ON p.id_brand = b.id_brand WHERE b.id_brand = %s",
            (id_brand,),
        )
